...
open LeanCrypto.Signature.Ed25519 (derivePublicKey sign verify)
open LeanCryptoVCVio

/-! M16 acceptance: `derivePublicKeyROM` / `signROM` / `verifyROM` (Ed25519
with SHA-512 modeled as a random oracle) produce the same byte-level output
as the standard scheme when the oracle is instantiated with the real
`LeanCrypto.Hash.SHA512.sha512`.

This is the "honest oracle" reduction step: in any actual UF-CMA proof the
random oracle would be a programmable abstraction whose distribution is
uniform on outputs; here we just demonstrate that the *modeling* faithfully
captures the standard scheme, which is exactly what M16 ships.

Universal `PerfectlyComplete` for the ROM variant is deferred for the same
reason as the non-ROM `ed25519` instance — see M15's commit message. -/
...
def main : IO UInt32 := do
  ...
  for v in rfcVectors do
    -- Run the ROM variant with the real SHA-512 wired in via sha512Impl.
    let pkOut  : ByteArray := simulateQ sha512Impl (derivePublicKeyROM v.sk) |>.run
    let sigOut : ByteArray := simulateQ sha512Impl (signROM v.sk v.msg) |>.run
    let okOut  : Bool      := simulateQ sha512Impl (verifyROM v.pk v.sig v.msg) |>.run
    checked := checked + 3
    if pkOut != v.pk then
      IO.eprintln ("FAIL derivePublicKeyROM " ++ v.name)
      failed := failed + 1
    if sigOut != v.sig then
      IO.eprintln ("FAIL signROM " ++ v.name)
      failed := failed + 1
    if okOut != true then
      IO.eprintln ("FAIL verifyROM " ++ v.name)
      failed := failed + 1
  if failed = 0 then
    IO.println ("OK " ++ toString checked ++
      " vectors (ROM Ed25519 with honest sha512 oracle matches RFC 8032 §7.1)")
    return 0
  else
